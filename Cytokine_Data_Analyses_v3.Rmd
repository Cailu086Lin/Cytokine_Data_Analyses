...
```{r import dat, echo=FALSE}

df<-read.csv("data/data_experimental_runs.csv", header=T)%>%
  mutate(Analyte=recode(Analyte, "IFN-g (25)"="IFN-¦Ã (25)", "TNF-a (75)"="TNF¦Á (75)","IL-1b (46)"="IL-1¦Â (46)" ))
    
```

#S1 Table
```{r runs corrections, echo=FALSE}
...
```{r data import}
df_relax<-read.csv("data/data_cleaned_ms.csv", header=T)%>%
  mutate(Analyte=recode(Analyte, "IFN-g (25)"="IFN-¦Ã (25)", "TNFa (75)"="TNF¦Á (75)","IL-1b (46)"="IL-1¦Â (46)" ))
```

S1_Table
```{r Table 1, demographics}
dem1<-read_excel("data/Copy of AERD_IDnoPHI_7-2020.xlsx")
dem2<-dem1[dem1$PtID %in% df_relax$ptid,]
...
S3 Table
```{r  S3_table, 2-way Scheirer-Ray-Hare tests, echo=FALSE}
...
write.csv(results, "S3_twowayResults.csv", row.names = F)

```

S3 Table
```{r posthoc Dunn test and adjusts FDR, echo=FALSE}
library(FSA)
...
S2_table
```{r S2_Table_correlation with covariates, echo=FALSE}
dem3<- read_excel("data/Copy of AERD_clinicalInfo_11-2021-1.xlsx")
...
pz<-cowplot::plot_grid(px,py, ncol=2, labels=NULL)
postscript(colormodel="cmyk")
ggsave(filename ='S1Figure_1QC_continue.tiff',pz, width =20, height =24, units ="cm",dpi = 600, device='tiff', limitsize = TRUE, compression = "lzw")
dev.off()
```


S5 Table . "correlation" among methods
```{r S5 Table sampling, echo=FALSE}
...
write.csv(corr_tbl, "S5_table_correlations_robust_sampling.csv", row.names = F)

```


Figure2A: drill down on epithelial brushings analysis of the cytokines
...
Fig2B: drill down on mucus analysis of the cytokines
comparing AERD (+/-steroid) 
```{r, Figure2B}
d2<-df_relax%>%
  filter(methods !="RIPAbuffer"&methods !="Background")%>%
...
Fig2<-cowplot::plot_grid(p1eb, p1m, ncol=2, label_size = 20, labels=c("A", "B"), rel_widths = c(3.3,1))
 
postscript(colormodel="cmyk")
ggsave(filename ='Figure_2.tiff',Fig2, width =32, height =15, units ="cm",dpi = 600, device='tiff', limitsize = TRUE, compression = "lzw")
dev.off()
```
S2 Figure: drill down on tissue and epi.bru analysis of the cytokines
comparing AERD (+/-steroid) 
```{r, S2 Figure}
d2<-df_relax%>%
  filter(methods !="RIPAbuffer"&methods !="Background")%>%
...
S2 Figure B: drill down on mucus analysis of the cytokines
comparing AERD (+/-steroid) 
```{r S2 Figure}
d2<-df_relax%>%
  filter(methods !="RIPAbuffer"&methods !="Background")%>%
...
S2Fig<-cowplot::plot_grid(p1a, p1b, ncol=2, label_size = 20, labels=c("A", "B"), rel_widths = c(1.9,1))
 
postscript(colormodel="cmyk")
ggsave(filename ='S2_Figure.tiff',S2Fig, width =26, height =15, units ="cm",dpi = 600, device='tiff', limitsize = TRUE, compression = "lzw")
dev.off()

```

#Figure 3,negative results IL13, CCL20, IL6 and TNFa in epitium sample
...
postscript(colormodel="cmyk")
ggsave(filename ='Figure_3.tiff',p1a, width =32, height =15, units ="cm",dpi = 600, device='tiff', limitsize = TRUE, compression = "lzw")
dev.off()
```


Supplemental Table 4
```{r tableOne for summary table, echo=FALSE}
# data_relax
d<-df_relax%>%
  rename(.,c("Obs.Conc"="Obs Conc"))%>%
  filter(methods=="mucus")%>%
...
write.csv(result, "S4_Table_Test_results_20210715.csv", row.names = FALSE)
```

#REviewer 2#15, combine brushings+mucus to and included it into S5 Table.
```{r brushings+mucus, echo=FALSE}
